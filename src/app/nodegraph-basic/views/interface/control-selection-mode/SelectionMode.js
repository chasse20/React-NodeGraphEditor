...
class SelectionControls extends React.Component
{
	constructor( tProps )
	{
		...
		// Events
		this._onPanOn = () => { this.props.options.isPanMode = true; };
		this._onPanOff = () => { this.props.options.isPanMode = false; };
	}
	
	render()
	{
		const tempIsPanning = this.props.options.isPanMode || this.props.graph.isPanning;
		
		return (
			<div className="select-toggle">
				<button className={ tempIsPanning ? "selected" : null } onMouseDown={ this._onPanOn }>
				...
				<button className={ this.props.options.isPanMode ? null : "selected" } onMouseDown={ this._onPanOff }>
